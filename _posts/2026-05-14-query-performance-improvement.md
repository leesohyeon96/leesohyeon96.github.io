...
API는 200을 반환하고 있었다. 하지만 29초 소요 후 빈 배열(`[]`). 앱 HTTP 타임아웃은 15초였다.

서버는 29초 후 정상적으로 쿼리를 완료했지만, 앱 쪽에서는 타임아웃이 먼저 발동해 연결을 끊은 상태였다. 클라이언트가 응답을 받지 못하고 빈 화면을 표시한 것이다. `body: []`가 서버 로그에 남은 정확한 이유는 확인하지 못했지만, 데이터 자체는 DB에 정상적으로 존재했다.
