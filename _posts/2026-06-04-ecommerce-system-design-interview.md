...
- Redis `SET NX`로 키 존재 여부 **빠른 사전 체크** (optional, DB 부하 감소 목적)
- **DB에 `idempotency_key` unique 제약 + 결제 결과 저장** ← 실제 중복 방지 보장

처리 흐름:
- 처음 요청 → DB INSERT 성공 → 결제 진행
- 중복 요청 → DB INSERT 실패 (unique 위반) → 저장된 이전 결과 조회 → 반환

Redis는 인메모리라 서버 재시작 시 키가 유실될 수 있다. 결제처럼 내구성이 중요한 경우 DB unique constraint가 최종 보장 수단이다. Redis는 외부 결제사 호출 전에 빠르게 걸러내는 pre-check 용도로만 쓴다.
...
[API 서버 (수평 확장)]
    ...
    │  [Redis Lua DECR]
    │     ↓ 성공
    │  [MySQL Primary — 직접 INSERT]  ← 평상시 (TPS ~260)
    │  [Kafka 주문 이벤트 발행]       ← 플래시세일 (TPS 2,600+)
    │     ↓
