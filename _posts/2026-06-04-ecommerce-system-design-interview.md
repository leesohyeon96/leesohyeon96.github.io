...
- 저장 **성공** (처음 요청) → 결제 처리 진행 → **처리 결과도 같은 키에 저장** (TTL 24시간)
- 저장 **실패** (중복 요청) → Redis에서 저장된 이전 결과 조회 → 그대로 반환, 재처리 없음

`SET NX`는 "키가 없을 때만 저장"하는 원자 명령어다. 동시에 두 요청이 들어와도 하나만 통과한다. 이전 결과를 반환하려면 처리 결과도 함께 Redis에 저장해야 한다.
