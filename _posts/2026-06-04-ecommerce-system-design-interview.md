...
**② 서버** — DB `idempotency_key` unique 제약으로 중복 방지

처리 흐름:
1. 처음 요청 → DB INSERT (`status=PROGRESS`) 성공 → 외부 결제사 API 호출
2. 결제사 응답 → DB `status=DONE` + 결과 저장
3. 중복 요청 → DB INSERT 실패 (unique 위반) → 기존 레코드 조회
   - `status=PROGRESS` → 처리 중 응답
   - `status=DONE` → 저장된 결과 그대로 반환
